...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (directPage) {
    ...
    return {
      title: directPage.title || `${serviceName}${cityName ? ` en ${cityName}` : ""} | Cerrajero Córdoba 24H`,
      description: directPage.meta_description || `Servicio profesional de ${serviceName.toLowerCase()}${cityName ? ` en ${cityName}` : ""}. Llama al 900 433 189.`,
      alternates: {
        canonical: `/${servicio}`,
      },
    }
  ...
  return {
    title: `${service.name} - Servicio Profesional en Toda España | Cerrajero Córdoba 24H`,
    description: service.description || `Encuentra profesionales de ${service.name} en tu ciudad. Llama al 900 433 189. Servicio garantizado y presupuesto gratis.`,
    alternates: {
      canonical: `/${servicio}`,
    },
  }
...
export default async function ServiceIndexPage({ params }: PageProps) {
  ...
  if (directPage) {
    ...
    // Build h1 with proper fallbacks
    const serviceName = serviceData.name || "Servicio"
    const cityName = cityData?.name || ""
    const defaultH1 = cityName ? `${serviceName} en ${cityName}` : directPage.title || serviceName

    const pageData = {
      title: directPage.title || defaultH1,
      h1: directPage.h1 || defaultH1,
    ...
    return (
      <ServiceCityPage
        ...
        service={serviceData}
        city={cityData}
        baseUrl="https://www.cerrajerocordoba.es"
      />
    )
  }
    ...
    <main className="min-h-screen bg-background">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="flex items-center justify-center">
              <div className="relative">
                ...
                <div className="relative bg-gradient-to-br from-orange-500 to-amber-500 rounded-3xl p-12 shadow-2xl">
      ...
      <section className="py-12 bg-gradient-to-r from-orange-500 to-amber-500">
        <div className="container mx-auto px-4 text-center">
          ...
          <Button size="lg" variant="secondary" className="gap-2 text-orange-600 font-semibold" asChild>
            <a href="tel:900433189">
              <Phone className="h-5 w-5" />
              900 433 189 - Llamada gratuita
            </a>
          </Button>
        </div>
      </section>
                    ...
                    <ul className="space-y-2">
                      {citiesByProvince[province].map((city) => (
                        <li key={city.pageSlug}>
                          <Link
                            href={`/${city.pageSlug}`}
                            className="flex items-center justify-between text-sm text-muted-foreground hover:text-orange-600 transition-colors group"
                          >
                            <span>{city.name}</span>
